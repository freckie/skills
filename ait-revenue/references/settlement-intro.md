...
정산은 서비스 운영과 수익 관리를 위해 반드시 이해해야 하는 과정이에요.\
이 문서에서는 정산 정보 등록부터 수익금을 실제로 받기까지의 흐름을 정리했어요.\
정산 구조를 미리 이해하면, 매출 확인부터 세금계산서 발행과 입금 일정까지 한 번에 관리할 수 있어요.
...
모든 정산은 앱 단위가 아니라 사업자 단위로 진행해요.\
즉, 하나의 사업자로 여러 앱을 운영하는 경우 각 앱의 수익과 비용을 합산해 정산해요.\
따라서 특정 앱의 매출만 따로 입금되지 않으며, 사업자 전체 기준으로 최종 정산 금액이 계산돼요.

![](/assets/console_settlement_1.Fl1UM9gy.webp)

![](/assets/console_settlement_2.Dbit1MM6.webp)
...
2026년 4월 30일까지 한시적으로 토스 수수료 0% 프로모션이 적용돼요.
...
정산 받을 수입이 **5,000원 이하**인 경우 다음 달로 이월하여 누적해요.\
일반과세자/간이과세자 구분은 콘솔에서 제휴사가 직접 선택해 주세요. 설정한 정보에 따라 정산이 진행되며, 정산 확정 이후에는 사업자 유형을 변경할 수 없어요.
...
![](/assets/console_settlement_iap.D51e0MlT.webp)
