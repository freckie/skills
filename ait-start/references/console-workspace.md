...
![](/assets/app_register_1.CmaExSlB.webp)
...
앱인토스를 이용하려면 먼저 앱인토스 콘솔에 가입해야 해요.\
[앱인토스 콘솔](https://apps-in-toss.toss.im/)에 접속해 회원가입을 진행해 주세요.\
앱인토스 콘솔 가입은 토스 비즈니스 회원을 기반으로 진행돼요.\
가입을 위해서는 **만 19세 이상**이어야 하고, **본인 명의로 로그인된 토스앱**이 필요해요.
...
사업자당 워크스페이스는 1개만 만들 수 있고, 이름은 중복으로 설정할 수 없어요.\
팀이나 프로젝트 이름처럼 쉽게 알아볼 수 있는 이름을 붙여 주세요. 나중에 수정할 수 있어요.

![](/assets/workspace_1.CV2fqS89.webp)

***

## 3. 멤버 초대하기

워크스페이스를 만든 사람에게 자동으로 '대표관리자' 권한이 부여돼요.

### 대표관리자의 역할

* 대표관리자는 멤버 초대 및 권한을 설정할 수 있어요.
...
![](/assets/member_invite_1.BFdduBhA.webp)
...
![](/assets/representative_1.C8M_4cNB.webp)

![](/assets/representative_2.VIfUXhXI.webp)
...
![](/assets/business_1.uqj6_WOX.webp)

![](/assets/business_2.BBPYZKsM.webp)
...
![](/assets/business_3.D3OtlddM.webp)
...
![](/assets/app_register_2.4B9Udbob.webp)

아래 정보만 입력하면 앱을 만들 수 있어요.\
나머지 앱정보는 준비되는 대로 입력해 주세요. 각 항목은 임시 저장할 수 있어요.\
다만 출시 전에는 필요한 앱정보를 모두 입력하고 검토를 요청해주세요.\
앱정보 검토가 끝나야 앱을 출시할 수 있어요.

![](/assets/app_register_3.Dan4nT8a.webp)

#### **① 앱 이름**

* 토스 앱에 노출되는 이름이므로, 서비스 성격이 바로 이해될 수 있도록 작성해 주세요.
* 기존 브랜드/서비스명을 사용 중이라면 그대로 쓰는 것을 권장해요.
* 앱 정보에서 나중에 수정할 수 있어요.
* 영문명은 아래 규칙을 따라 주세요.
  * 15자 이내의 발음하기 쉬운 명사형 이름으로 작성해요.
  * 명령문이나 동사구는 사용하지 않아요.
    * 'Book a Taxi'가 아닌 'GoRide Taxi'처럼 브랜드명 형태로 작성
...
* 만들려는 앱이 게임인지 비게임인지에 따라 앱 유형을 선택해 주세요.
* 선택한 앱 유형에 따라 이후에 입력해야 하는 앱정보 항목이 달라져요.

### 4-1) 기본 정보

![](/assets/app_register_4.DRpXEAbD.webp)

#### **① 부제**

* 사용자가 어떤 서비스를 이용할 수 있는지 쉽게 이해할 수 있도록 간결하게 작성해 주세요.
* 전체 메뉴에서 검색했을 때 미니앱 이름 하단 등에 활용될 수 있어요.
* 비속어 및 느낌표 등은 사용할 수 없어요.

#### **② 상세 설명**

상세 설명을 바탕으로 토스 홈 광고 대상으로 선정될 수 있어요.

* 사용자가 서비스에서 경험할 수 있는 내용을 최대한 많이, 구체적으로 적어 주세요.
* 사용자가 서비스를 처음 실행한 뒤 핵심 기능을 경험하기까지의 과정을 적어 주세요.
  * 서비스 접속 → 행동 → 결과 흐름에 따라 문장으로 풀어서 설명해 주세요.
* 사용자가 실제로 무엇을 보고, 무엇을 누르며, 어떤 경험을 하게 되는지를 중심으로 자세하게 작성해 주세요.

:::details **예시: 장학금 정보 서비스**

1. 필터를 제공해요. → 해외 장학금, 거주 지역, 가구원 정보를 기반으로 필터를 제공해요.
2. 알림 신청을 걸어두면 나에게 딱 맞는 장학금 정보를 푸시로 안내해요. → 알림 신청을 걸어두면 거주지역, 가구원 정보를 기반으로 나에게 딱 맞는 장학금에 대해서 금액과 접수기한에 대해 푸시로 안내해요.
   :::
...
* 번개/화살/주먹 등의 무기 사용
* 2번의 턴마다 무기 고르기 가능
* 10번 몬스터 무찌르면 골드 20개 수령
* 우측 출석 버튼 누르면 첫 출석 시 100개 다이아 보상 수령
  :::
...
![](/assets/app_register_5.C_5-nIne.webp)

![](/assets/app_register_6.Blx0LMjQ.webp)
...
* 게임 앱인 경우, 출시하는 게임의 장르에 맞는 카테고리를 선택해요.
  * 웹보드 게임에 해당하는 경우, 반드시 웹보드 여부를 체크해야 해요.\
    [웹보드 게임 서비스 주의사항 바로가기](/intro/caution.html)
...
![](/assets/app_register_7.CJ8fy5ut.webp)
...
* 다른 앱과 혼동되지 않아야 하고, 저작권 문제가 없어야 해요.
* 다크 모드에서도 자연스럽게 보이는 색상과 형태를 사용해 주세요.
* 정사각형 600 × 600px, PNG(배경색 필수, 투명 배경 사용 불가) 파일이어야 해요.
* 토스에서 제공하는 아이콘 및 이미지 리소스 등을 앱 로고로 사용하는 것은 불가해요. (2차 가공 포함)

#### **④ 썸네일 이미지**

썸네일 이미지는 토스 앱 내 다양한 지면 등에 활용되며,\
사용자가 서비스에 접속하기 전 가장 먼저 보게 되는 이미지예요.\
사용자가 어떤 서비스인지 직관적으로 이해할 수 있고, 긍정적인 인상을 받을 수 있도록 제작해 주세요.

**썸네일 이미지 주의사항**

* 1932 x 828px, PNG 파일로 업로드해 주세요.
...
* 텍스트는 과도하게 사용하지 말고, 필요하다면 로고나 짧은 문구만 적절히 배치해요.
...
**\[Do]**
![](/assets/thumbnail_1.Bon3jiBg.webp)

**\[Don't]**

![](/assets/thumbnail_2.C72qb_iH.webp)

![](/assets/thumbnail_3.Cf9wjmCw.webp)

![](/assets/thumbnail_4.jyqH-sAJ.webp)
...
* 스크린샷 이미지는 자체 제작 이미지 또는 스크린샷 형태 모두 가능해요.
* 검색 결과 및 앱 미리 보기에 활용되므로, 미니앱의 특징을 한눈에 파악할 수 있는 이미지를 사용해 주세요.
* 이미지 입력은 선택 사항이에요. 입력한다면 **세로형일 때 최소 3장, 가로형일 때 최소 1장**을 업로드해 주세요.
  * **세로형 규격 사이즈:** 636 x 1048px, PNG파일
  * **가로형 규격 사이즈:** 1504 x 741px, PNG파일

#### **⑥** 리더보드 (게임 앱)

![](/assets/game_1.CjYumZ2C.webp)
...
![](/assets/game_intro_1.FXD3b7tr.webp)
